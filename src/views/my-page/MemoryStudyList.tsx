...
import {StudyPost, StudyPostData} from "@/views/MemoryStudy.tsx";
import Pagination from "@/components/Pagination.tsx";
import {useNavigate} from "react-router-dom";


const MemoryStudyList = ()=>{
    ...
    const getMemoryStudyList =async ()=>{
        const result = await Get<{ data: { data:StudyPost } }>(`/v1/myInfo/certification?size=10&page=${page+1}&sort=RegDt,DESC`)
        console.log(result)
    ...
    return <div>
        <div className={'flex flex-col gap-[10px]'}>
            {
                list.map((post) => <div className={'flex items-center gap-[10px] bg-white border border-[#E6E6E6] px-[16px] py-[10px] rounded-[20px] cursor-pointer'} onClick={ ()=> navigate(`/detail/${post.boardSid}?page=memory-study`)}>
...
export default MemoryStudyList;